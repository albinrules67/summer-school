...
.ovg{padding:24px}
.dm{position:fixed;inset:0;z-index:300;background:rgba(0,0,0,.8);backdrop-filter:blur(8px);display:none;align-items:center;justify-content:center;padding:20px}
.dm.s{display:flex}
.dmc{background:var(--bg2);border:var(--bw);border-radius:16px;max-width:520px;width:100%;max-height:90vh;overflow-y:auto;animation:dmf .3s}@keyframes dmf{from{opacity:0;transform:translateY(20px)}}
.dmh{position:sticky;top:0;background:var(--bg2);padding:16px 20px;border-bottom:var(--bw);display:flex;align-items:center;justify-content:space-between;z-index:2}
.dmht{font-weight:700;color:#fff;font-size:15px}
.dmx{padding:8px;border-radius:8px;background:transparent;border:1px solid rgba(255,255,255,.08);color:var(--fg2);cursor:pointer;font-size:18px;line-height:1;transition:.2s}.dmx:hover{color:#fff}
.dmi{background:linear-gradient(180deg,rgba(255,255,255,.02),transparent);display:flex;align-items:center;justify-content:center;padding:24px}
.dmi img{max-width:100%;max-height:280px;object-fit:contain;filter:drop-shadow(0 8px 24px rgba(0,0,0,.6))}
.dmd{padding:0 20px 20px}
.dmd b{display:block;font-size:13px;color:var(--fg2);font-weight:500;text-transform:uppercase;letter-spacing:.5px}
.dmd h2{font-size:22px;font-weight:800;color:#fff;margin:4px 0 16px;letter-spacing:-.3px}
.dmdc{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px}
.dmdl{border:var(--bw);border-radius:10px;padding:12px;background:var(--bg3)}
.dmdl span{display:block;font-size:9px;color:var(--fg2);text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px}
.dmdl strong{font-size:14px;font-weight:700;color:#fff}
.dmb{display:flex;gap:8px;margin-top:16px}
.dmb a{padding:10px 18px;border-radius:8px;border:1px solid rgba(235,75,75,.25);color:var(--r1);font-size:13px;font-weight:600;text-decoration:none;text-align:center;transition:.2s;flex:1}
.dmb a:hover{background:rgba(235,75,75,.08)}
.dmb .dmb2{border-color:rgba(255,255,255,.08);color:var(--fg2)}.dmb .dmb2:hover{color:#fff}
</style>'''
# --- END CSS ---
